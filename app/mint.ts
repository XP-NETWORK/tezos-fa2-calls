...
(async () => {
  ...
  const storage = await xpnftContract.storage<CollatzStorage>()
  // @ts-ignore
  console.log(storage.all_tokens.toNumber())
  ...
  try {
    const tokenId = 0
    let op1 = await collatzContract.methods.mint(metadata, owner).send({
      fee: 100_000,
      gasLimit: 800_000,
      storageLimit: 60_000,
    })
